...
while True:
	...
	for (objectID, centroid) in objects.items():
		...
		# to determine direction
			...
			for line in linesV:
				#print(list(zip(np.linspace(line[0],line[2], 10), np.linspace(line[1],line[3], 10))))
				#negative x direction <- go to the left
				#meanx > line and direction negative this line count one car in the left direction
				if directionx < 0 and to.linecounted==False and (centroid[0] in range(line[0],line[2])) and ((centroid[1] in range(line[1],line[3]))):
					print(objectID,directionx,centroid,line)
					to.linecounted=True
					print("He contado izquierda")
				#positive x direction -> go to the right
				#meanx < line and direction positive  this line count one car in the right direction
				if directionx > 0 and to.linecounted==False and (centroid[0] in range(line[0],line[2])) and ((centroid[1] in range(line[1],line[3]))):
					print(objectID,directionx,centroid,line)
					to.linecounted=True
					print("He contado derecha ")
			for line in linesH:
				#negative y direction  go up
				#meany > line and direction negative this line count one car going up
				if directionx < 0 and centroid[1] < line[1] and to.centroids[-1][1] > line[1]:
					print("He contado arriba")
				#positive y direction -> go down
				#meany < line and direction positive  this line count one car going down
				if directionx > 0 and centroid[1] > line[1] and to.centroids[-1][1] < line[1]:
					print("He contado abajo")
			to.linecounted=False
