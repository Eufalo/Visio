...
while True:
	...
	for (objectID, centroid) in objects.items():
		...
		# to determine direction
			...
			countv=0
			counth=0
			#we need to wit 10 frames to take over the flag
			# If we dont count this trackable object the array it´s empty
			#we put the flag to false when skyp 10 frame over the last  counting
			if  to.framecounted and  to.framecounted[-1]< totalFrames-20:
				to.flag_count=False
			#loop over vertical lines
			while countv < len(linesV) and to.flag_count==False :
				#If the centroid cross between lineX1 to lineX2 and between lineY1 to lineY2
				if (centroid[0] in range(linesV[countv][0],linesV[countv][2])) and ((centroid[1] in range(linesV[countv][1],linesV[countv][3]))):
					#negative x direction <- go to the left
					if directionx < 0:
						to.flag_count=True
						to.framecounted.append(totalFrames)
	...
	# if the `v` key was pressed, u can add a vertical line
			...
			print(trackableObjects.get(1).centroids)
